...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    Dodd-Frank / 12 CFR 252 & DFAST CET1 floor (adverse) & 4.5\% (regulatory) & 8.7\% trough \\
    Basel IV / FRTB & FRTB-SA floor & Binding from 2025 & --- \\
...
\subsection{DFAST Stress Constraint}
...
\begin{itemize}[noitemsep]
  ...
  \item $B_{\text{CCB}} = 2.5\%$: Capital Conservation Buffer
  \item $B_{\text{DFAST}}$: buffer between trough (severely adverse) and minimum;
        at Apex this is $\approx 2.2\%$ (trough 6.7\%, minimum 4.5\%)
...
reserves (Fed Funds: 5.25\%), and Treasury bills/bonds (4.5\%--4.7\%).
The optimization shifts toward Treasuries and away from excess cash.
...
\subsection{Hedging the Duration Gap}

If the current duration gap exceeds the target, the ALM desk uses interest rate
swaps to close the gap:

\begin{itemize}[noitemsep]
  \item \textbf{Pay-fixed receiver IRS:} Converts a floating-rate liability
        into fixed-rate, increasing effective liability duration.
  \item \textbf{Receive-fixed payer IRS:} Converts a fixed-rate asset into
        floating, reducing effective asset duration.
\end{itemize}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Duration trap}: a sharp rate rise (+300bps rapidly) reduces
        EVE to near-zero while simultaneously increasing funding costs on
        short-dated liabilities (SVB-type scenario).
...
DFAST) are not merely compliance requirements — they are \emph{priced inputs}
...
At Apex Global Bank, the infrastructure described in this document — the ALM
engine, FTP engine, DFAST engine, XVA suite, SIMM collateral engine, and
regulatory capital engine — provides the quantitative foundation for this
optimization program.  The SR 11-7 model governance framework ensures that
the outputs of these models are reliable, independently validated, and used
